...
def send_http(s,info,target,target_port):
	...
	if method == 'GET':
		r = s.get(url,timeout=timeout,headers=request_header,allow_redirects=False)
	else:
		r = s.post(url, data=request_body,timeout=timeout,headers=request_header,allow_redirects=False)
